...
bool our::ShaderProgram::attach(const std::string &filename, GLenum type) const {
    ...
    string error = checkForShaderCompilationErrors(shader); 
    if (!error.empty()) {
        cerr << "ERROR: Couldn't compile shader: " << filename << std::endl;
        cerr << error << std::endl;
...
bool our::ShaderProgram::link() const {
    ...
    string error = checkForLinkingErrors(program);
    if (!error.empty()) {
        cerr << "ERROR: Couldn't link shader program" << std::endl;
        cerr << error << std::endl;
        return false;
    }
